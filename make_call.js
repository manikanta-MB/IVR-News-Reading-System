...
let remoteUrl = "https://e6d9-160-238-73-167.ngrok.io/";
...
app.get("/call", (req, res) => {
  const userPhoneNumber = req.query.to || process.env.TO_NUMBER;
  let ncco = [];
  ncco.push(
    getTalkAction(
      userInfo,
...
app.post("/main_menu_input", (req, res) => {
  ...
  if (entered_digit == "") {
    ...
    let userPhoneNumber = responseObject.to;
...
app.post("/request_category", (req, res) => {
  ...
  if (requestObj.speech.timeout_reason == "start_timeout") {
    ...
    let userPhoneNumber = requestObj.to;
